...
# The unit starts the server with --config, which insists the named file exists,
# and the admin web interface writes the administrator's password hash into it.
# An empty object is a valid config that changes nothing, so this only gives the
# server somewhere to write. An existing file is never touched: it carries that
# password hash after the first sign in.
if [ -d /etc/relay-tunnel ] && [ ! -e /etc/relay-tunnel/config.json ]; then
    echo '{}' > /etc/relay-tunnel/config.json
    chmod 0640 /etc/relay-tunnel/config.json
fi
