...
func quicConfig() *quic.Config {
	// Keep-alives keep a healthy link alive well within the idle timeout, so the
	// idle timeout only fires when the peer has genuinely gone away (crash or
	// network loss). A relatively short idle timeout lets the client detect a
	// dead peer and reconnect promptly.
	return &quic.Config{
