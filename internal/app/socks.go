...
// arbitrary outbound reach through the peer. It takes a ConnSource so the
// listener survives reconnections.
func SOCKS(ctx context.Context, src ConnSource, localAddr string) error {
	...
	for {
		...
		go serveSOCKS(ctx, src, c)
	}
}

func serveSOCKS(ctx context.Context, src ConnSource, c net.Conn) {
	defer c.Close()
