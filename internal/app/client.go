...
func LocalForward(ctx context.Context, conn *quic.Conn, spec string) error {
	...
	for {
		...
		go func() {
			defer c.Close()
			s, err := openTCP(ctx, conn, remoteTarget)
			if err != nil {
...
// openTCP opens a KindTCP stream to the given target.
func openTCP(ctx context.Context, conn *quic.Conn, target string) (*quic.Stream, error) {
