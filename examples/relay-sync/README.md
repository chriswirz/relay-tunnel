...
Flags may be given before or after the command, so `relay-sync --config sync.json push ./local remote` and `relay-sync push --config sync.json ./local remote` are equivalent.
