...
func cmdServer(args []string) {
	...
	if err := runServer(httpAddr, udpAddr, publicUDP, publicURL); err != nil {
		xlog.Fatalf("server: %v", err)
	}
}

// runServer starts the rendezvous server and blocks.
func runServer(httpAddr, udpAddr, publicUDP, publicURL string) error {
	s := sig.NewServer(sig.ServerConfig{HTTPAddr: httpAddr, UDPAddr: udpAddr, PublicUDP: publicUDP, PublicURL: publicURL})
	return s.Run()
}
...
func runExpose(ctx context.Context, server, code string, pol app.HostPolicy) error {
	...
	for ctx.Err() == nil {
		err := hostOnce(ctx, server, code, cert, fp, pol)
		if ctx.Err() != nil {
			break
		}
...
func cmdAuto(args []string) {
	...
	if cfg.Server.Enabled {
		...
		modes = append(modes, mode{"server", func() error {
			return runServer(orDefault(sc.HTTP, ":7000"), orDefault(sc.UDP, ":7001"), sc.PublicUDP, sc.PublicURL)
		}})
	}
