...
rem   build.cmd --web      export the admin web interface into internal\webui\out
rem   build.cmd --examples build everything under examples\ for this machine
rem   build.cmd --all      build the web interface, then cross-compile every release target into dist\
...
if /i "%~1"=="--web"  set "PS_ARGS=%PS_ARGS% -Web"  & shift & goto parse
if /i "%~1"=="--examples" set "PS_ARGS=%PS_ARGS% -Examples" & shift & goto parse
